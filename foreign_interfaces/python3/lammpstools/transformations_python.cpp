...
PYBIND11_PLUGIN(transformations_) {
	...
	m.def("rotate_all", &lt_transformations_rotate_all,
	      "Rotates all particles over an axis at given origin.");
	m.def("shift_all", &lt_transformations_shift_all,
	      "Shifts all particles over given distance.");
	m.def("center_box_on", &lt_transformations_center_box_on,
	      "Centers the entire box on given point.");

	m.def("shift", &lt_transformations_shift,
	      "Shifts given particles over given distance");
	m.def("rotate", &lt_transformations_rotate,
